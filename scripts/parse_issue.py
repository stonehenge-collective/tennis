...
def parse_issue_body(body):
    ...
    sets_match = re.search(r"### Sets \(one line per set, winner’s games first\)\s*\n\s*```\n(.*?)\n```", body, re.DOTALL)
    if sets_match:
    ...
    if not data.get('sets'):
        ...
        for i, s in enumerate(data['sets']):
            if not (isinstance(s, list) and len(s) == 2 and all(isinstance(x, int) for x in s)):
                errors.append(f"Set #{i+1} has an invalid format. It must be `<WinnerGames>-<LoserGames>` (e.g., '6-3').")
